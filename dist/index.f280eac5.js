...
if (window.location.pathname === "/") {
    const photographers1 = _dataJsonDefault.default.photographers;
    ...
    dispPhotographers(photographers1, document.querySelector(".photographers"));
    ...
    // 2 implicit returns
    const filterPhotographersByTag1 = (photographers, tag)=>photographers.filter((photographer)=>photographer.tags.includes(tag)
        )
    ;
    const addClickToTags = ()=>{
        ...
        for(let i = 0; i < children.length; i++){
            ...
            li.addEventListener("click", (e)=>{
                const filteredPhotographers = filterPhotographersByTag(photographers1, e.target.textContent.slice(1));
                dispPhotographers(filteredPhotographers, document.querySelector("section"));
            });
        }
    };
    addClickToTags();
    ...
    const { photographers  } = _dataJsonDefault.default;
    const photographer1 = photographers.find((photographer)=>photographer.id === parseInt(id) ? photographer : null
    );
    console.log(photographer1.name);
}
